...
const SolicitudesAusencias = () => {
  ...
  const handleMostrarMensajeConfirmacion = (dato) => {
    ...
    setMensajeUsuario(
      'Está por eliminar la solicitud de ausencia con fecha inicial: ' +
        formatearFecha(dato.fechaIni) +
        ' y fecha fin: ' +
        formatearFecha(dato.fechaFin)
    )
  ...
  const handleAceptarBorrado = (e) => {
    ...
    fetch(
      `${URL_BASE}eliminar-periodo-ausencia/` + solicitudPorBorrar.id,
      requestOptions
    )
      .then((response) => response.json())
      .then((data) => {
