...
const EditFechaYHoraController = ({
  ...
  const [startDate, setStartDate] = useState(
    moment(reserva.fecha, 'YYYY-MM-DD').toDate()
  )
      ...
      <div id="selectorhora">
        ...
        {horaInicio === '' ? (
          <select name="" id="" onChange={(e) => setHoraInicio(e.target.value)}>
            {horas.map((hora, i) => {
              if (hora === reserva.horaIni) {
                return (
                  <option selected key={hora} value={hora}>
                    {hora}
                  </option>
                )
              } else {
        ...
        {horaFinal === '' ? (
          <select name="" id="" onChange={(e) => setHoraFinal(e.target.value)}>
            {horas.map((hora, i) => {
              if (i < posInicial(reserva.horaIni) + 1) {
                return ''
              } else {
                if (hora === reserva.horaFin) {
                  return (
                    <option selected key={hora} value={hora}>
                      {hora}
                    </option>
