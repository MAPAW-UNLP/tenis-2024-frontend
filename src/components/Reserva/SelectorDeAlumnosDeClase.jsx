...
const SelectorDeAlumnosDeClase = ({
  alumnos,
  setAlumnosDeLaClase,
  alumnosDeLaClase,
}) => {
  ...
  const yaEsta = (alumno) => {
    return alumnosDeLaClase.filter((a) =>
      a.nombre.toUpperCase().includes(alumno.nombre.toUpperCase())
    )
  }
  ...
  return (
    <div>
      <input
        ...
        name=""
        placeholder="Nombre"
        id="buscadorDeAlumnos"
        onChange={handleChangeAlumnos}
      />
      <div id="alumnosListAddDiv">
        {alumnosFiltrados.map((el, index) => {
          return yaEsta(el).length === 0 ? (
            <div key={index} className="clase-detail-a" id="alumnosListAdd">
              <p onClick={() => handleAddAlumno(el)}>{el.nombre}</p>{' '}
            </div>
          ) : (
            <div
              key={index}
