...
export const BalanceTable = ({ balance }) => {
  return (
    <>
      <div
        className="container container-balance"
        style={{
              ...
              <Fragment key={`${item.id}-${item.movimiento_id}`}>
                <tr className="table-row">
                  <td style={{ textAlign: 'center' }}>
                    {item.fecha_format} - {item.hora} hs
                  </td>
                  <td>{item.concepto_desc}</td>
                  <td>{item.nombre}</td>
                  <td>{item.descripcion}</td>
                  <td className="debe-td">
                    {item.movimiento_id === 1 && `$${item.monto}`}
                  </td>
                  <td className="haber-td">
                    {item.movimiento_id === 2 && `$${item.monto}`}
                  </td>
                </tr>
                <tr style={{ height: '5px' }} />
              </Fragment>
      ...
      <div className="balance-table-footer">Total: ${balance.total}</div>
    </>
  )
}
