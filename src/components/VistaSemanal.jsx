  ...
  return (
    <div>
      <div
        ...
        style={{
          display: 'grid',
          gridTemplateColumns: `repeat(${canchas.length + 1}, 1fr)`,
      ...
      >
        ...
        {semana.map((el, i) => (
          <div
            className="semana"
            key={el}
            style={{ gridArea: `1/${i + 2} / ${horas.lenght}/${i + 3}` }}
          >
            <div id="semana-nombre" style={{ height: '4vh' }}>
              {' '}
              {el}
            </div>{' '}
          </div>
        ...
        {reservas.map((el) => (
          <ReservaSemanal key={el.reservaId} datos={el} canchas={canchas} />
        ))}
      </div>
    </div>
  )
